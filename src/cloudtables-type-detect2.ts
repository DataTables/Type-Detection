...
export default class typeDetect {
				...
				if(dateSuggestion !== null && format.momentFormat !== dateSuggestion.momentFormat) {
					for(var j = 0; j < i; j++){
						if(!moment(data[j], format.momentFormat).isValid()) {
							console.log("previous does not pass", data[j], format.momentFormat)
							return 'mixed';
						}
					}
				}
	...
	public getDateFormat(el: string, suggestion, definiteOrder):any {
		...
		if(suggestion !== null && format.split.length !== suggestion.split.length) {
				console.log("different lengths", format.split, suggestion.split);
				return "mixed";
		}
		...
		for(let i = 0; i < format.split.length; i++) {
			if(
				...
				console.log("PASS", format.format[i], suggestion.format[i])
				...
				if(format.format[i].value === "DD" || format.format[i].value === "D" || format.format[i].value === "Do" ) {
					...
					console.log("hasMonth", 243)
				}
				continue;
			}
			else if(suggestion !== null) {
				console.log("FAIL", format.format[i], suggestion.format[i])
				// format.format[i] = suggestion.format[i];
			}
		...
		if(!format.hasDay && !format.hasMonth && !format.hasYear && missing === 3) {
			...
			for(let empty of empties) {
				if(!isNaN(+format.split[empty])) {
					console.log(+format.split[empty])
			...
			if(possMonth.length === 1) {
				if(format.split[possMonth[0]].indexOf("0") === 0) {
					format.format[possMonth[0]].value = {value:"MM", definite:true, firm:false}
				}
				else {
					format.format[possMonth[0]].value = {value:"MM", definite:true, firm:false}
				}
					...
					if(!format.hasDay || !format.hasYear || !format.hasMonth) {
						if(format.hasYear && format.hasDay) {
							if(format.split[i].indexOf("0") === 0) {
								format.format[i] = {value: "MM", definite: true, firm: true}
								format.hasMonth = true;
								console.log("hasMonth", 398)
							}
							else {
								format.format[i] = {value: "M", definite: true, firm: false}
								format.hasMonth = true;
								console.log("hasMonth", 403)
							}
						}
							...
							if(format.split[i].indexOf("0") === 0) {
								...
								format.hasYear = true;
								console.log("has day and month", format)
							}
							else {
							...
							console.log("check remaining day")
							if(format.split[i].indexOf("0") === 0) {
								format.format[i] = {value: "DD", definite: true, firm: true}
								format.hasDay = true;
								console.log("hasDay", 417)
							}
							else {
								format.format[i] = {value: "D", definite: true, firm: false}
								format.hasDay = true;
								console.log("hasDay", 422)
							}
						}
							...
							if(format.split[i].indexOf("0") === 0) {
								format.format[i] = {value: "MM", definite: true, firm: true}
								format.hasMonth = true;
								console.log("hasMonth", 434)
							}
							else {
								format.format[i] = {value: "M", definite: true, firm: false}
								format.hasMonth = true;
								console.log("hasMonth", 439)
							}
						}
							...
							if(format.split[i].indexOf("0") === 0) {
								format.format[i] = {value: "MM", definite: true, firm: true}
								format.hasMonth = true;
								console.log("hasMonth", 446)
							}
							else {
								format.format[i] = {value: "M", definite: true, firm: false}
								format.hasMonth = true;
								console.log("hasMonth", 451)
							}
						}
					}
				}
		...
		console.log(format.momentFormat)

		if(moment(el, format.momentFormat).isValid()) {

			return format;
		}
		else {
			console.log("not valid", el, format.momentFormat)
			return "mixed";
		}
	}
